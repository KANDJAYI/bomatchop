-- Adresse de livraison saisie par le client au checkout (domicile / lieu convenu).
alter table public.orders
  add column if not exists delivery_address text;

comment on column public.orders.delivery_address is 'Adresse ou consignes de livraison à domicile (saisie client au checkout).';
...
begin
  ...
  loop
    ...
    values (v_uid, v_status, p_payment, v_total, v_delivery)
...
grant execute on function public.create_orders_split_by_vendor(public.payment_method, jsonb, text) to authenticated;
